  ...
  group('calculateScanWindowRelativeToTextureInPercentage', () {
    group('can compute scan window for landscape widget inside portrait texture', () {
      ...
      test('with BoxFit.fitHeight', () {
        ctx.testScanWindow(BoxFit.fitHeight, const Rect.fromLTRB(0, 0.25, 1, 0.75));
      });
      ...
      test('with BoxFit.contain', () {
        ctx.testScanWindow(BoxFit.contain, const Rect.fromLTRB(0, 0.25, 1, 0.75));
      });

      test('with BoxFit.scaleDown', () {
        ctx.testScanWindow(BoxFit.scaleDown, const Rect.fromLTRB(0, 0.25, 1, 0.75));
      });
    });

    group('can compute scan window for landscape widget inside landscape texture', () {
...
class ScanWindowTestContext {
  ...
  void testScanWindow(BoxFit fit, Rect expected) {
    final actual = ScanWindowUtils.calculateScanWindowRelativeToTextureInPercentage(
      fit,
      scanWindow,
      textureSize: textureSize,
      widgetSize: widgetSize,
